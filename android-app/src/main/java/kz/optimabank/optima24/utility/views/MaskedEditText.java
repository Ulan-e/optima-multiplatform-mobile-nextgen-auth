...
//					return false;
    ...
    public void cleanUp() {
        ...
        if (hasHint()) {
            ...
            StringBuilder text = new StringBuilder(mask.replace(charRepresentation, SPACE_CHARACTER));
            String defaultText = this.getText().toString();
            int number = 0;
            for (int i = 0; i < text.length(); i++) {
                if (text.charAt(i) == SPACE_CHARACTER) {
                    text.setCharAt(i, number >= defaultText.length() ? SPACE_CHARACTER : defaultText.charAt(number));
                    number++;
                }
            }
    ...
    private String makeMaskedText() {
        ...
        for (int i = 0; i < rawToMask.length; i++) {
            if (i < rawText.length()) {
                ...
                maskedText[rawToMask[i]] = SPACE_CHARACTER;
            }
        }
